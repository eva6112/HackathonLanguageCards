...
struct StatsView: View {
    ...
    var chartData: [(day: String, count: Int)] {
        ...
        return grouped.map { (day: $0.key, count: $0.value) }.sorted { $0.day < $1.day }
    }
    ...
    var body: some View {
        NavigationStack {
                    ...
                    if !chartData.isEmpty {
                        ...
                        Chart {
                            ForEach(chartData, id: \.day) { item in
                                BarMark(x: .value("День", item.day), y: .value("Слова", item.count)).foregroundStyle(Color.blue.gradient)
                            }
                        }
            ...
            .navigationTitle("Прогресс").navigationBarTitleDisplayMode(.inline).toolbar { Button("Закрыть") { dismiss() } }.background(Color(UIColor.systemGroupedBackground))
        }
    }
}
...
struct StatBox: View {
    ...
    var body: some View {
        VStack(spacing: 8) {
            ...
            Text(title).font(.footnote).foregroundColor(.secondary).multilineTextAlignment(.center)
        }
        .frame(maxWidth: .infinity, minHeight: 90).background(Color(UIColor.secondarySystemGroupedBackground)).cornerRadius(15).shadow(color: .black.opacity(0.05), radius: 5)
    }
}
